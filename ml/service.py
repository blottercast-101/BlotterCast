...
def load_incidents() -> pd.DataFrame:
    """Loads active, verified incident reports from the database."""
    with db_engine.connect() as conn:
        df = pd.read_sql(
            text(
                "SELECT id, incident_date AS date, time_reported, zone_id AS zone, hour, category, priority, status "
                "FROM incidents WHERE archived = 0 OR archived IS NULL ORDER BY incident_date"
            ),
            conn,
        )
    if not df.empty:
        df['date'] = pd.to_datetime(df['date'])
        # Filter strictly to official 7 zones
...
def train():
    df = load_incidents()
    if len(df) < 10:
        return jsonify({'ok': False, 'error': 'Not enough incident records to train models (minimum 10 required)'}), 400

    panel = build_spatiotemporal_panel(df)
    if panel.empty or len(panel) < 20:
        return jsonify({'ok': False, 'error': 'Insufficient temporal span across zones for panel generation'}), 400
...
def latest():
    """Returns the evaluated model insights and forecasts directly from memory."""
    ...
    with db_engine.connect() as conn:
        ...
        row = result.mappings().first()
    if not row:
        return jsonify({'ok': False, 'message': 'No trained models available yet. Retrain model to initialize.'}), 404
